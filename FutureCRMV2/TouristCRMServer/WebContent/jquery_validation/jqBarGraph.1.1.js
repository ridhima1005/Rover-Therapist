 ...
 * 
 * @param data:
 *            arrayOfData // array of data for your graph
 * @param title:
 *            false // title of your graph, accept HTML
 * @param barSpace:
 *            10 // this is default space between bars in pixels
 * @param width:
 *            400 // default width of your graph ghhjgjhg
 * @param height:
 *            200 //default height of your graph
 * @param color:
 *            '#000000' // if you don't send colors for your data this will be
 *            default bars color
 * @param colors:
 *            false // array of colors that will be used for your bars and
 *            legends
 * @param lbl: '' //
 *            if there is no label in your array
 * @param sort:
 *            false // sort your data before displaying graph, you can sort as
 *            'asc' or 'desc'
 * @param position:
 *            'bottom' // position of your bars, can be 'bottom' or 'top'. 'top'
 *            doesn't work for multi type
 * @param prefix: '' //
 *            text that will be shown before every label
 * @param postfix: '' //
 *            text that will be shown after every label
 * @param animate:
 *            true // if you don't need animated appereance change to false
 * @param speed:
 *            2 // speed of animation in seconds
 * @param legendWidth:
 *            100 // width of your legend box
 * @param legend:
 *            false // if you want legend change to true
 * @param legends:
 *            false // array for legend. for simple graph type legend will be
 *            extracted from labels if you don't set this
 * @param type:
 *            false // for multi array data default graph type is stacked, you
 *            can change to 'multi' for multi bar type
 * @param showValues:
 *            true // you can use this for multi and stacked type and it will
 *            show values of every bar part
 * @param showValuesColor:
 *            '#fff' // color of font for values
 * 
...
(function($) {
	...
	$.fn.jqBarGraph = $.fn.jqbargraph = function(options) {
		...
		doGraph = function(el) {
			...
			legendWidth = arr.legend ? arr.legendWidth : 0; // width of legend
															// box
			fieldWidth = ($(el).width() - legendWidth) / data.length; // width
																		// of
																		// bar
			...
			for ( var val in data) {
				...
				out = "<div class='graphField" + el.id + "' id='graphField"
						+ unique + "' style='position: absolute'>";
				out += "<div class='graphValue" + el.id + "' id='graphValue"
						+ unique + "'>" + prefix + value + postfix + "</div>";

				out += "<div class='graphBar" + el.id + "' id='graphFieldBar"
						+ unique + "' style='background-color:" + color
						+ ";position: relative; overflow: hidden;'></div>";

				// if there is no legend or exist legends display lbl at the
				// bottom
				if (!arr.legend || arr.legends)
					out += "<div class='graphLabel" + el.id
							+ "' id='graphLabel" + unique + "'>" + lbl
							+ "</div>";
				out += "</div>";
				...
				totalHeightBar = totalHeight
						- $('.graphLabel' + el.id).height()
						- $('.graphValue' + el.id).height();
				...
				if (valueData instanceof Array) {
					...
					for (i in valueData) {
						...
						o = "<div class='subBars" + el.id + "' style='height:"
								+ heig + "px; background-color: "
								+ arr.colors[i] + "; left:" + wid * i
								+ "px; color:" + arr.showValuesColor
								+ "; font-size:" + fs + "px' >" + sv + "</div>";
		...
		createLegend = function(legendArr) {
			...
			for ( var val in legendArr) {
				legend += "<div id='legend" + legendArr[val][3]
						+ "' style='overflow: hidden; zoom: 1;'>";
				legend += "<div class='legendBar" + legendArr[val][2]
						+ "' id='legendColor" + legendArr[val][3]
						+ "' style='background-color:" + legendArr[val][0]
						+ "'></div>";
				legend += "<div class='legendLabel" + legendArr[val][2]
						+ "' id='graphLabel" + unique + "'>"
						+ legendArr[val][1] + "</div>";
				legend += "</div>";
			}
		};
