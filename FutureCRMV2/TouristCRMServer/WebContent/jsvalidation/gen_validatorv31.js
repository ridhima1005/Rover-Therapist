...
function form_submit_handler2() {
	var bRet = true;
	document.error_disp_handler.clear_msgs();
	for ( var itr = 0; itr < document.forms[0].elements.length; itr++) {
		if (document.forms[0].elements[itr].validationset
				&& !document.forms[0].elements[itr].validationset.validate()) {
			bRet = false;
		}
		if (!bRet && !document.forms[0].show_errors_together) {
			break;
		}
	}
	if (!bRet) {
	...
	if (document.forms[0].addnlvalidation) {
		str = " var ret = " + document.forms[0].addnlvalidation + "()";
		eval(str);
		if (!ret)
			return ret;
	}
...
function validateInput(strValidateStr, objValue, strError) {
	...
	case "num":
	case "numeric": {
		ret = TestInputType(objValue, "[^0-9]", strError, objValue.name
				+ ": Only digits allowed ");
		break;
	}
	case "dec":
